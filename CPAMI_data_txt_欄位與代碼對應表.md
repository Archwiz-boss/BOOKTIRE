...
舊程式另在密碼保護的 `Build.mdb`／`Bldcode` 表保存畫面用預設資料。此來源和上表的 `bldcode.mdb` 是兩份不同快照，前端只讀提取後合併使用，不應直接覆寫任一 MDB：

| CODE_TYPE | 原始筆數 | 實際用途 |
|---|---:|---|
| `RMK` | 141 | 臺中市規定備註；52 列程序／屬性加 89 列可帶入全文，整理成 6 個分類 |
| `BMLAW1` | 43 | `BMSBASE.LAW_01` 防火及防火避難適用版本；其中 `**` 是表頭，不是可寫入代碼 |
| `BMLAW2` | 6 | `BMSBASE.LAW_03` 耐震設計規範適用版本；其中 `**` 是表頭，不是可寫入代碼 |
| `MEMO` | 53 | 舊系統常用文字；目前保留在代碼檔供後續畫面擴充 |

`RMK` 的 `MARK` 是上層分類，程序列提供「程序、屬性代碼／名稱」，內容列再以相同程序關聯到備註全文。寫回案件時分類不落欄，只成組寫入 `BMSMEMO.MEMO_SEQ`、`MEMO_SEQ_NAME`、`DESE`。舊資料存在少數分類名稱與內容列標記異體（例如「部分／部份」）；匯出工具只做已驗證的別名連結，不改動來源資料。
...
| 100 | `LAW_01` | Text | 2 | 1/1 | 41 | 防火及防火避難適用版本；查 `BMLAW1` |
| 101 | `LAW_02` | Text | 7 | 0/1 | （空） | 防火避難性能設計認可日期；民國 7 碼，不是法規版本代碼 |
| 102 | `LAW_02_DOC` | Text | 10 | 0/1 | （空） | 防火避難性能設計認可通知書文號；依個案輸入 |
| 103 | `LAW_03` | Text | 2 | 1/1 | 05 | 耐震設計規範適用版本；查 `BMLAW2` |
